...
public class CoreBean implements CoreBeanLocal {
    ...
    public void createWeek(String weekAsJson) throws JsonKeyInFaultException, PersistentException, InvalidTokenException, UserDontExistsException, ClientAlreadyHasAnPlanException, PlanDontExistException, InvalidWeekNumberException {
        ...
        // Verify if PersonalTrainer exists and if not create him (is the first week of plan that builds)
        PersonalTrainer pt;
...
// Verify if createWeek has called to create a new plan or to add a new week to an existing plan
        if(!json.has("planId")) {
        ...
        else {//Here dont need to associate Client and PersonalTrainer because here adds a new week to an existing plan already associated with the previous ones
            ...
            // get the lastWorkout to check if is done, if yes then the new week becomes the currentWeek
