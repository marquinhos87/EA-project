...
public class Utils {
    ...
    public static void validateToken(String token, String username) throws TokenIsInvalidException, PersistentException {
        List results = NotificationFacade.getSession().createQuery("select token from User where username='" + username + "' and token='" + token + "'").list();
